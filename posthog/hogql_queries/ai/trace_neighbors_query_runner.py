...
class TraceNeighborsQueryRunner(AnalyticsQueryRunner[TraceNeighborsQueryResponse]):
    ...
    def _calculate(self):
        ...
        older_trace_id, older_timestamp, newer_trace_id, newer_timestamp = self._execute_neighbors_query(
            self._build_query(primary)
        )

        # Fall back for missing directions
        if older_trace_id is None or newer_trace_id is None:
            fb_older, fb_older_ts, fb_newer, fb_newer_ts = self._execute_neighbors_query(self._build_query(fallback))
